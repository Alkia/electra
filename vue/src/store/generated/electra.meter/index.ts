	...
	actions: {
		...
		async QueryCustomerbillingline({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ElectraMeter.query.queryCustomerbillingline( key.customerDeviceID,  key.cycleID,  key.lineid,  key.paid)).data
		...
		async QueryGetcustomerbill({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ElectraMeter.query.queryGetcustomerbill( key.customerDeviceID,  key.billCycleID)).data
		...
		async QueryProducerbillingline({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ElectraMeter.query.queryProducerbillingline( key.producerDeviceID,  key.cycleID,  key.lineid,  key.paid)).data
		...
		async QueryGetproducerbill({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ElectraMeter.query.queryGetproducerbill( key.producerDeviceID,  key.billCycleID)).data
