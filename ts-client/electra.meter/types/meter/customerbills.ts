...
const baseCustomerbills: object = {
  billCycleID: 0,
  customerdeviceID: "",
  billDate: 0,
  billTotalWh: 0,
...
export const Customerbills = {
  encode(message: Customerbills, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Reader | Uint8Array, length?: number): Customerbills {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseCustomerbills } as Customerbills;
  ...
  fromPartial(object: DeepPartial<Customerbills>): Customerbills {
    const message = { ...baseCustomerbills } as Customerbills;
    if (object.billCycleID !== undefined && object.billCycleID !== null) {
